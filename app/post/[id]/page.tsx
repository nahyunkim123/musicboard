...
export default async function Detail({ params }: { params?: { id?: number } }) {
  ...
  const [results] = await db.query<RowDataPacket[]>(
    'select * from musicboard.board1 where id = ? ',
    [postId],
  );
  ...
  const [countResult] = await db.query<RowDataPacket[]>(
    'select count (*) as cnt from musicboard.view_log where postid = ? and ip_address = ?',
    [postId, userIp],
  );
  ...
  if (results.length > 0) {
    if (totalCnt === 0) {
      await db.query<RowDataPacket[]>(
        'update musicboard.board1 set count = count + 1 where id = ? ',
        [postId],
      );
    }

    await db.query<RowDataPacket[]>(
      'insert into musicboard.view_log (postid, ip_address, view_date) select ?, ?, NOW() where not exists (select 1 from musicboard.view_log where postid = ? and ip_address = ? and view_date > now() - interval 24 hour)',
          ...
          <div className="w-4/5 p-7 border rounded-xl mx-auto">
            <div className="w-full flex justify-between items-end  pb-3">
              <div className="text-md">
                <span className="font-bold">{post.username}</span>님의 플레이리스트
              </div>
              ...
              {session ? (
                ...
                <>
                  <p className="block border p-4 text-center my-5 rounded-md">
                    <Link href="/login">댓글을 작성하려면 로그인해주세요</Link>
                  </p>
                </>
              )}
