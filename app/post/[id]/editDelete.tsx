...
export default function EditDelete({ results }: propsType) {
  ...
  return (
    <>
      {session && session.user && results && results && session.user.email === results.userid && (
        <>
          <div className="w-full mt-7 flex justify-between items-end pb-3">
            <button className="py-2 px-3 border rounded-md">
              <Link href={`/edit/${results?.id}`}>수정</Link>
            </button>
            <button
              className="py-2 px-3 border rounded-md bg-red-400 text-white"
              onClick={() => deletePost(results?.id)}
            >
              삭제
            </button>
          </div>
