  ...
  return (
    <>
      ...
      <div className="widget w-full overflow-hidden">
        {data &&
          data.map((e: memberInfo, i: number) => {
            ...
            return (
              <ul
                className="flex border-b last:border-0 justify-between text-center py-2 text-xs sm:text-sm md:text-base"
                key={i}
              >
                ...
                <li className="basis-2/12">
                  <Link href={`/admin/member/edit/${e.id}`}>수정</Link>/
                  <MemberDelete name={e.name} id={e.id} />
                </li>
              </ul>
            );
          })}
