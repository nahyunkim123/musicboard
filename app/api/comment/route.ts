    ...
    try {
      ...
      if (!username || !username || !parentid || !content) {
        ...
        const [results] = await db.query<RowDataPacket[]>(
          'insert into musicboard.comment ( userid, username, parentid, content) values(?,?,?,?)',
          [userid, username, parentid, content],
        );
        const [datas] = await db.query<RowDataPacket[]>(
          'select * from musicboard.comment where parentid = ?',
          [parentid],
        );
...
export const GET = async (req: NextRequest): Promise<NextResponse> => {
  if (req.method === 'GET') {
    try {
      ...
      const [results] = await db.query<RowDataPacket[]>(
        'select * from musicboard.comment where parentid = ? order by date DESC',
        [parentid],
      );
