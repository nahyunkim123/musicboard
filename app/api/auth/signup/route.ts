...
export const POST = async (req: NextRequest): Promise<NextResponse> => {
  if (req.method === 'POST') {
    ...
    const [checkMember] = await db.query<RowDataPacket[]>(
      'select count(*) as cnt from musicboard.member where email = ?',
      [email],
    );
    ...
    if (memberCnt > 0) {
      ...
      await db.query(
        'insert into musicboard.member (email, password, name, nickname, gender) values(?,?,?,?,?)',
        [email, hash, name, nickname, gender],
      );
