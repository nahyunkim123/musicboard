            ...
            <div className='max-w-7xl text-center'>
                ...
                <form method="post"  className='w-4/5 p-5 mt-8 mx-auto border h-auto rounded-md' onSubmit={submitEvent
                    ...
                    <div className='flex items-center'>
                        ...
                        <input className='py-2 px-2 basis-5/6 border rounded-xl focus:outline-[#999]' defaultValue={ post && post[0]?.title} type="text" name="title" onChange={changeEvent} />
                    </div>
                    <textarea className='border p-3 w-full h-[600px] mt-4 rounded-xl focus:outline-[#999]' name="content"  defaultValue={post && post[0]?.content} onChange={changeEvent}/>

                    <div className="flex justify-between">
                    <button className="bg-[#FA7070] transition-all text-white px-4 py-2 rounded-xl shadow-md focus:outline-none">
                        <Link href='/'>취소</Link>
                        </button>
                        <button className="bg-[#3C486B] transition-all text-white px-4 py-2 rounded-xl shadow-md focus:outline-none" onClick={()=>submitEvent}>업로드</button>
                    </div>
                </form>
            </div>
        </>
