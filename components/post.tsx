    ...
    <>
      <div className="mx-auto max-w-7xl p-6">
        <div className="flex justify-around items-center mb-6 relative">
          <h1 className="text-[80px] font-semibold">게시판</h1>
          <Link
            href="/write"
            className="bg-[#3C486B] absolute -bottom-[15px] transition-all text-white px-4 py-2 rounded-xl shadow-md right-3 hover:bg-[#3F497F]"
          >
            글쓰기
          </Link>
        </div>
      ...
      <div className="w-4/5 p-6 mx-auto mt-9 border rounded-md">
        {posts &&
          posts.map((e, i) => {
            ...
            return (
              <ul
                className="mb-5 flex justify-between border-b items-center transition-all  hover:bg-slate-200 hover:rounded-md"
                key={i}
              >
                <li className="rounded-md border px-3 py-1 bg-[#3C486B] text-white basis-1/14 text-center">
                  {posts.length - i}
                </li>
                <li className="px-6 py-3 basis-8/14 text-center">
            ...
            return (
              <button
                ...
                className={`border px-5 py-1 ${
                  page === pageNum ? 'bg-black text-white font-bold' : 'bg-white text-black'
                }`}
